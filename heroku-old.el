...
(defun heroku-domains-list ()
  "List all environment variables on a Heroku app."
  (interactive)
  (let* ((app-name (heroku--read-app-name))
         (buffer-name (format "*Heroku Config: %s" app-name)))
    (message (format "Getting config variables for %s" app-name))
    (make-comint-in-buffer "heroku-config" buffer-name "heroku" nil "domains" "-a" app-name)
    (switch-to-buffer-other-window buffer-name)))

(defun heroku-org--list-domains (app-name)
  "List all domains for an app."
  (->>
   (shell-command-to-string (concat "heroku domains -a " app-name))
   (replace-regexp-in-string (concat app-name " ") "")
   (replace-regexp-in-string "===" "***")))

(defun heroku-org--list-config (app-name)
  (let ((lst (->>
              (shell-command-to-string (concat "heroku config -a " app-name))
              (replace-regexp-in-string "=== .+" "")
              (split-string)

              ;; (map (lambda (s) (replace-regexp-in-string ":s+" " " s)))
              ;; (concat "| Name | Value |")
              )))
    (lax-plist-get lst "AWS_ACCESS_KEY_ID:")))

(comment
 (setq temp (heroku-org--list-domains "clj-blaster"))
 (->> (heroku-org--list-config "clj-blaster")
      (mapc 'message))
 temp
 (lax-plist-get '("AWS:" "foo") "AWS:")
 ...
 "=== clj-blaster Config Vars
AWS_ACCESS_KEY_ID:                             AKIAWPXTKLLI4AZV5AA6
...
(defun heroku-org--make-app-buffer ()
  "Create an org-mode buffer with information and settings of a Heroku app."
  (interactive)
  (let* ((app-name (heroku--read-app-name)) (buffer-name (format "*Heroku App: %s" app-name)))
    (get-buffer-create buffer-name)
    (switch-to-buffer-other-window buffer-name )
    (erase-buffer)
    (org-mode)
    (org-insert-heading)
    (insert "Heroku App Management: " app-name)
    (newline 2)
    (org-insert-subheading nil)
    (insert "Domains")
    (newline)
    (insert (heroku-org--list-domains app-name))
    (org-insert-heading nil)
    (org-promote)
    (insert "Environment Variables")
    ;; (start-process "heroku-domains" buffer-name "heroku" "domains" "-a" "clj-blaster")
    ))

(comment
 (heroku-org--make-app-buffer)
 herbuff
 )
